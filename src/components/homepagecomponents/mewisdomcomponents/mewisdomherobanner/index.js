import React from "react";
import "./mewisdomherobanner.scss";

export default function Mewisdomherobanner() {
  return (
    <div className="me-wisdom-herobanner-main">
      <div className="me-wisdom-herobanner-content">
        <div className="container1">
          <h1>ME wisdom</h1>
          <p>
            Lorem ipsum dolor sit amet consectetur. Et quam porttitor faucibus
            nunc sagittis. Aliquet vestibulum tristique maecenas sit interdum
            tincidunt lacinia. Fermentum faucibus morbi est nec tellus vulputate
            ut et. Dictum aliquam egestas vitae purus et ut nec scelerisque.
            Velit eros tempor at ut eleifend eu mattis dictumst. Sapien nec erat
            turpis imperdiet. Aenean diam laoreet vulputate cras facilisi ut
            netus.
          </p>
        </div>
      </div>
    </div>
  );
}
